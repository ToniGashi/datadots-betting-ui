...
const MatchContainer = ({operator, match, name, sport}) => {
    ...
    return (
        <div className="flex justify-left w-fit mb-10">
            {(home && away) && <div className="mx-8 mt-8 content-center self-center">
                ...
                <div className='flex mb-5 w-60'> {/*Lower this to bring coeficients closer */}
                    <img key={home} src={`/logos/${home.replace(' ', '').toLowerCase()}.jpg`} width="30" height="20" className='mr-2 h-10 w-10 self-center' alt='flag' 
                ...
                <div className='flex mb-5 w-60'>
                    <img key={away} src={`/logos/${away.replace(' ', '').toLowerCase()}.jpg`} width="30" height="20" className='mr-2 h-10 w-10 self-center' alt='flag'
                        onError={({currentTarget}) => {
                            currentTarget.onerror = null; 
                            currentTarget.src=`/logos/${sport.replace(' ', '').toLowerCase()}.jpg`
                            }}/>
